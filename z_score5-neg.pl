...
my $seq_dir = "RF00023_60_blastclust_sub1\\negative";
my $updated;
my $neg_dir = "z_score";
my $err_file = "fallout.txt";
my $seq_file = "file2-2.txt";
my $score_file;
my $j;
my $pos_mfe;
my $pos_rnaali;
...
#my @allfiles = grep {/combo_[0-9]*_4_[0-9]*-ran.txt/} readdir THISDIR;
#closedir THISDIR;
...
unless ( open(FALLOUT, ">> $seq_dir\\$err_file") ) {

	print "Cannot open file \"$seq_dir\\$err_file\"\n\n";
	exit;
}
...
#	print "stddev $stddev\n";
	...
	if ($stddev != 0) {
		...
		$z = ($pos_mfe - $mean)/$stddev;

	}

	else {

		print FALLOUT "$pos_file\n";
